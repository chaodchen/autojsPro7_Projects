...
配置.str.颜色 = '#DDC700'
配置.str.名称 = '#抖音私信脚本'
配置.str.公告 = 'QQ：934082222'
配置.str.日志 = '/sdcard/'+配置.str.代号+'_log.txt'

//新建本地储存
var storage = storages.create(配置.str.代号)

//绘制UI
ui.layout(
<vertical>
    <appbar>
        <toolbar id="toolbar" title="{{this.配置.str.名称}}" bg='{{this.配置.str.颜色}}'/>
        <tabs id="tabs" bg='{{this.配置.str.颜色}}'/>
    </appbar>
    <viewpager id="viewpager">
        <frame>
            <vertical>
                <horizontal gravity='center_vertical'>
                    <text text='' margin='5dp' bg='{{this.配置.str.颜色}}' w='8dp' h='35dp'></text>
                    <text text='软件权限' textSize='16sp' textColor='{{this.配置.str.颜色}}'></text>
                </horizontal>
                ...
                <horizontal gravity='center_vertical'>
                    <text text='' margin='5dp' bg='{{this.配置.str.颜色}}' w='8dp' h='35dp'></text>
                    <text text='必读公告' textSize='16sp' textColor='{{this.配置.str.颜色}}'></text>
                </horizontal>
                <card cardCornerRadius='5dp' margin='5dp' cardElevation='5dp'>
                    <text padding='10dp' text='{{this.配置.str.公告}}'></text>
                </card>
            </vertical>
        </frame>
        <frame>
                ...
                <card cardCornerRadius='5dp' margin='5dp' cardElevation='5dp'>
                    <vertical>
                        <vertical padding='5dp'>
                            <text text='发送话术：'></text>
                            <input id='input_0' w='*'></input>
                        </vertical>
                        <vertical padding='5dp'>
                            <text text='发送延迟：'></text>
                            <input id='input_1' w='*'></input>
                        </vertical>
                    </vertical>
                </card>
            </vertical>
...
function 开始 () {
    ...
    log('UI配置-->>  '+配置.UI配置)
    new Function('配置', 配置.fun.getCloudFiles(配置.str.路径+'window.js', 配置.str.密钥))(配置)
}
...
//这里的配置变量是从main脚本文件传递过来的
log(配置)

配置.名称 = '抖音私信回复'
配置.公告 = 'QQ：934082222'
配置.日志 = '/sdcard/'+配置.代号+'.txt'
配置.颜色 = '#FF1493'
...
ui.emitter.on("options_item_selected", (e, item)=>{
    switch(item.getTitle()){
        case "开始":
            ...
            storage.put('UI配置', UI配置)
            threads.start(function(){
                new Function('配置', 配置.函数.getCloudFiles(配置.路径+'window.js', 配置.密钥))(配置)
            })
            break;
        case "日志":
