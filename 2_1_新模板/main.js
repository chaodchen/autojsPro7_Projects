...
const BH = (function(){
    ...
    BH.prototype.drawUiFrame = function () {
        ui.layout(
            <frame w='*' h='*'>
                <fab visibility='gone' layout_gravity='right|bottom' id='fab' margin='15dp' layout_width='wrap_content' layout_height='wrap_content' backgroundTint='{{bh.主题颜色}}' elevation='5dp' src="@drawable/ic_send_black_48dp" color='{{bh.主题颜色}}'></fab>
    ...
    BH.prototype.searchFun = function (keys) {
        let result = [];
        for (let i = 0; i < bh._json.script.length; i++) {
            let sname = bh._json.script[i];
            if (sname.title.indexOf(keys) >= 0) {
                result.push(sname);
            }
        }
        ui.run(() => {
            ui.listView.setDataSource(result);
        });
    }
 ...
 * 
 */
bh = new BH("17685034710@163.com", "ah94esu8ymqvir9m", "TaoBao/2_1_新模板/");
...
let pjy = null, pjysdk = null;

let th = threads.start(function(){
    //开始获取云端json文件
    bh._json = bh.getJgy("config.json").str();bh._json = JSON.parse(bh._json);
    bh.标题 = bh._json.style.标题;
    bh.副标题 = bh._json.style.副标题;
    bh.公告 = bh._json.style.公告;
    bh.主题颜色 = bh._json.style.主题颜色;
    bh.jgy = bh._json.style.会员系统.开关;
    bh.pjyKey = bh._json.style.会员系统.key;
    bh.pjySecret = bh._json.style.会员系统.secret;
