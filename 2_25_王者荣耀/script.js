...
var storage = storages.create("2_25_王者荣耀");

var 主线任务 = storage.get("bh_zhuxian") || 0;

var 运行时间 = storage.get("bh_yunxing") || 60;

var 点击坐标 = storage.get("bh_zb");
...
function 连续点击 (坐标组) {
    threads.start(function(){
        ...
        坐标组.split("-").forEach((coor) => {
            if (coor && coor.indexOf("|") > -1) {
                let x = coor.split("|")[0];
                let y = coor.split("|")[1];
                toastLog("x:"+x+", y:"+y);
                if (device.sdkInt > 23) {
...
function run_script () {
    ...
    this.while_click_coor_array = function (coorArr, delayTime, coorRange) {
        ...
        this.click_coor = function (coor) {
            ...
            if (device.sdkInt > 23) {
                ...
                let sh = shell("input tap "+x+" "+y, true);
                log("Root:%d", sh.code);
            }
    ...
    switch (task) {
        case 0:
            ...
            var coorArr = [];
            coorArr.push(device.height * 0.7984 + "|" + device.width * 0.9148);
            coorArr.push(device.height * 0.7671 + "|" + device.width * 0.8333);
        ...
        case 1:
            toastLog("武道大会");
            var coorArr = [];
            coorArr.push(device.height * 0.8114 + "|" + device.width * 0.7009);
            coorArr.push(device.height * 0.6166 + "|" + device.width * 0.4314);
            coorArr.push(device.height * 0.9093 + "|" + device.width * 0.9259);
            coorArr.push(device.height * 0.4927 + "|" + device.width * 0.9361);
