...
const BH = (function(){
    function BH (user, key, path, pjyKey, pjySecret) {
        ...
        log("storage:"+this.getJgyProjectName(path , 2));
        this.home = "";
        this.setTing = "";
        this.pjyKey = pjyKey;
    ...
    BH.prototype.drawUiFrame = function () {
        ui.layout(
            <frame w='*' h='*'>
                <img scaleType='centerCrop' w='*' h='*' src='http://aj.8888-8.cn/appImg/2_25_王者荣耀/bg.jpg'></img>
        ...
        bh.scriptTh = threads.start(function(){
            ...
            if (bh.悬浮窗) {
                ...
                let window = floaty.window(
                    <frame>
                        <img id='action' scaleType='centerCrop' src='http://aj.8888-8.cn/appImg/2_25_王者荣耀/window.png' w='35dp' h='35dp'></img>
                        <text id='txt' visibility='gone' text='显示'></text>
                    </frame>
                );
                ...
                function onClick () {
                    if (window.txt.text() == "显示") {
                        ui.run(() => {
                            window.txt.setText("隐藏");
                            toastLog("显示");
                        });
                    } else if (window.txt.text() == "隐藏") {
                        ui.run(() => {
                            window.txt.setText("显示");
                            toastLog("隐藏");
                        });
                ...
                if (bh.runScriptTh == null) {
                    bh.runScriptTh = threads.start(function(){
                        let script = bh.getJgy("script.js").str();
                        engines.execScript("script", script);
                    });
                } else {
                    toastLog("脚本已经在运行")
                }
...
bh = new BH("17685034710@163.com", "ah94esu8ymqvir9m", "TaoBao/2_25_王者荣耀/", "c0rqrdco6ita2il2phn0", "o0jFWT5VVlrxPiya66cEXJEt3wszkhSK");
