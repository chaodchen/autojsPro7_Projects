...
编辑框控件组 = [
    [{text:'点赞次数：'}, {inputType:'number'}],
    [{text:'关注次数：'}, {inputType:'number'}],
    [{text:'转发次数：'}, {inputType:'number'}],
    [{text:'从第几号分身开始运行：'}, {inputType:'number'}],
    [{text:'全局延迟：'}, {inputType:'number', hint:'单位为秒钟！'}],
...
if (函数库路径 != null || 函数库路径 != '') {
    ...
    获取函数线程 = threads.start(function(){
        downloadCloudFiles(函数库路径, 配置.密钥, 配置.账号, 'mods.js')
    })
