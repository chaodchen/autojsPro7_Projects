 ...
 * @LastEditors: BanHua
 * @LastEditTime: 2021-01-14 14:56:30
...
var userUI = (function(){
    return (
        <vertical>
            <horizontal>
                <text text='操作延迟：'></text>
                <input hint='x-x秒/默认为2-5秒' w='*' id='bh_delay'></input>
            </horizontal> 
            <horizontal>
                <text text='操作平台：'></text>
                <input hint='爱兼职|58兼职' w='*' id='bh_czpt'></input>
            </horizontal> 
            <horizontal>
                <text text='操作次数：'></text>
                <input inputType='number' w='*' id='bh_czcs'></input>
            </horizontal>
        </vertical>
    )
})();
        ...
        scriptThreads = threads.start(function(){
            ...
            function windowButtonClick () {
                if (window.scriptState.getText() == '开始') {
                    ...
                    log('开始获取脚本资源');
                    let _jgyStr = null;
                    let _newTh = threads.start(function(){
                        _jgyStr = GetJgyFile(CONFIG.jgyUser, CONFIG.jgyKey, CONFIG.jgyPath+"script.js");
                    });
                    while(_newTh.isAlive());

                    
                    // console.log("_jgyStr"+_jgyStr);
