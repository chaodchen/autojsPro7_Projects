/**最后修订与2020年12月29日13点11分 */


var 下滑延迟 = storage.get('viewDataArr')['xhyc'];
var 上滑 = storage.get('viewDataArr')['sh'];
var 下滑 = storage.get('viewDataArr')['xh'];
var 在线时间 = storage.get('viewDataArr')['zxsj'];
...
threads.start(function(){
    在线时间 = strMods.randomNumOfSing(在线时间, '-');
    toastLog('本次在线时间为：'+在线时间+'分钟！');
 ...
 * 开启线程
 */
var 开始线程 = threads.start(KsVideoAutoPagerChrome);
...
function KsVideoAutoPagerChrome(){
    ...
    while (1) {
        ...
        // var 已下滑次数 = 0;
        // var 已上滑次数 = 0;
        下滑次数 = strMods.randomNumOfSing(下滑, '-');
        log('下滑次数：'+下滑次数);
        for (let i = 0; i < 下滑次数; i++) {
            log('开始for循环下滑任务');

            var time = VideoRandomTime(下滑延迟.split('-')[0], 下滑延迟.split('-')[1]);
            toastLog("下一视频，请等待" + time/1000 +"秒");
            sleep(time);  
            //检测青少年模式
        ...
        上滑次数 = strMods.randomNumOfSing(上滑, '-');
        for (let i = 0; i < 上滑次数; i++) {
            log('开始for循环上滑任务');
