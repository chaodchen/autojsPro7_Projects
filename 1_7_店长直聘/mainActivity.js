...
const PATH = engines.myEngine().cwd()+'/script.js';
//声明主题颜色
const COLOR = '#FFD700';
//声明脚本标题
const TITLE = '店长招人脚本';
...
//需要保存的空间ID
var viewIdArr = ['hfyc', 'zpgw', 'zprj'];

//声明本地储存
storage = storages.create('dzzp');
...
threads.start(function () {
    scriptFun = new Function('', files.read(PATH));
});
...
var UI = ui.inflate(
    <vertical>
        <horizontal>
            <text text='招聘软件：'></text>
            <spinner id='zprj' entries='店长直聘|同城兼职'></spinner>
        </horizontal>
        <horizontal>
            <text text='回复延迟：'></text>
            <input id='hfyc' hint='4-6' w='*'></input>
        </horizontal>
        <horizontal>
            <text text='招聘岗位：'></text>
            <input id='zpgw' hint='岗位一|岗位二' w='*'></input>
        </horizontal>
